...
class ClassDef extends Statement {
    ...
    *iteratePrimitives() {}
    ...
    get isdb() {
        return this.loader ? this.loader.module === 'org.thingpedia.database' : false;
    }

    get db_server() {
        return this.isdb ? this.loader.in_params.find((param) => param.name === 'db_server').value.toJS() : false;
    }
    ...
    toManifest() {
        ...
        if (this.loader.in_params.length > 0)
            this.loader.in_params.forEach((in_param) => { manifest[in_param.name] = in_param.value.toJS(); });
