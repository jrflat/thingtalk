...
function extractImports(manifest) {
    ...
    if (manifest.module_type) {
        let param = [];
        if (manifest.module_type === 'org.thingpedia.database' && manifest.db_server)
            param.push(new InputParam('db_server', fromJS(manifest.db_server)));
        imports.push(new ImportStmt.Mixin(['loader'], manifest.module_type, param));
    }
    let argmap = {};
